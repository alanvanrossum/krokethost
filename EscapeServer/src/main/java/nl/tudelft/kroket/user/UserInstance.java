...
public class UserInstance implements Runnable {
	...
	case Protocol.COMMAND_REGISTER:
		...
		RegisteredUser newPlayer = EscapeHost.registerPlayer(clientSocket,
			outputStream, playerName);
	...
	case Protocol.COMMAND_ADMIN:
		...
		adminCommand(input
			.substring(Protocol.COMMAND_ADMIN.length() + 1));
	    break;
	default:
